...
Bagshui.config.Inventory = {
	...
	[BS_INVENTORY_TYPE.BAGS] = {
		...
		-- with the Backpack.
		bagButtonIdOffset = (
			(select(1, _G.GetInventorySlotInfo("Bag0Slot")) or select(1, _G.GetInventorySlotInfo("BAG0SLOT")) or 20) - 1
		),
	...
	[BS_INVENTORY_TYPE.BANK] = {
		...
		-- Epoch is based on the 3.3.5 client, so reserve enough buttons for the
		-- larger bank plus all purchasable bank bag slots.
		initialItemSlotButtons = 28 + ((_G.NUM_BANKBAGSLOTS or 6) * 20),

		-- Append all bank bag containers when containerIds is built at
		-- the end of this file.
		containerIdRange = { 5, 4 + (_G.NUM_BANKBAGSLOTS or 6) },
		-- XML template for bag buttons.
