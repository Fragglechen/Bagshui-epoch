...
-- https://github.com/Fragglechen/Bagshui-epoch
--
...
-- Epoch runs on a 3.3.5-derived client, but some addons ship container APIs
-- through C_Container while older addons still expect the global functions.
local originalContainerIDToInventoryID = _G.ContainerIDToInventoryID
if _G.C_Container then
	_G.GetContainerNumSlots = _G.GetContainerNumSlots or _G.C_Container.GetContainerNumSlots
	_G.GetContainerItemInfo = _G.GetContainerItemInfo or _G.C_Container.GetContainerItemInfo
	_G.GetContainerItemLink = _G.GetContainerItemLink or _G.C_Container.GetContainerItemLink
	_G.GetContainerItemCooldown = _G.GetContainerItemCooldown or _G.C_Container.GetContainerItemCooldown
	_G.PickupContainerItem = _G.PickupContainerItem or _G.C_Container.PickupContainerItem
	_G.UseContainerItem = _G.UseContainerItem or _G.C_Container.UseContainerItem
end

_G.ContainerIDToInventoryID = function(containerId)
	if not _G.GetInventorySlotInfo then
		if originalContainerIDToInventoryID then
			local ok, inventoryId = pcall(originalContainerIDToInventoryID, containerId)
			if ok then
				return inventoryId
			end
		end
	...
	if containerId == 0 then
		return select(1, _G.GetInventorySlotInfo("Bag0Slot")) or select(1, _G.GetInventorySlotInfo("BAG0SLOT"))
	end

	if type(containerId) == "number" and containerId >= 1 and containerId <= (_G.NUM_BAG_SLOTS or 4) then
		local bagSlotNum = containerId - 1
		return
			select(1, _G.GetInventorySlotInfo("Bag" .. bagSlotNum .. "Slot"))
			or select(1, _G.GetInventorySlotInfo("BAG" .. bagSlotNum .. "SLOT"))
	end

	if type(containerId) == "number" and containerId >= 5 and containerId <= (4 + (_G.NUM_BANKBAGSLOTS or 6)) then
		local bankBagSlotNum = containerId - 4
		return
			select(1, _G.GetInventorySlotInfo("Bag" .. bankBagSlotNum))
			or select(1, _G.GetInventorySlotInfo("BAG" .. bankBagSlotNum))
	end

	if containerId == _G.KEYRING_CONTAINER and _G.KeyRingButtonIDToInvSlotID then
		return _G.KeyRingButtonIDToInvSlotID(1)
	end
...
local bagshuiEnvironment = {
	...
	-- Bagshui homepage.
	BS_URL = "github.com/Fragglechen/Bagshui-epoch",
	-- Bagshui wiki root. MUST end in a slash.
	BS_WIKI_URL = "github.com/Fragglechen/Bagshui-epoch/wiki/",
...
function Bagshui:Init()
	...
	self.eventFrame:SetScript("OnEvent", function(_, event, arg1, arg2, arg3, arg4)
		self:OnEvent(event, arg1, arg2, arg3, arg4)
	end)
...
function Bagshui:OnEvent(event, arg1, arg2, arg3, arg4)
	...
	local previousEvent = self.currentEvent
	local previousEventArg1 = self.currentEventArg1
	local previousEventArg2 = self.currentEventArg2
	local previousEventArg3 = self.currentEventArg3
	local previousEventArg4 = self.currentEventArg4

	self.currentEvent = event
	self.currentEventArg1 = arg1
	self.currentEventArg2 = arg2
	self.currentEventArg3 = arg3
	self.currentEventArg4 = arg4
	...
	if event == "ADDON_LOADED" then
		...
		if arg1 == "Bagshui" then
			...
			self.currentEvent = previousEvent
			self.currentEventArg1 = previousEventArg1
			self.currentEventArg2 = previousEventArg2
			self.currentEventArg3 = previousEventArg3
			self.currentEventArg4 = previousEventArg4
			return
		end
	end
