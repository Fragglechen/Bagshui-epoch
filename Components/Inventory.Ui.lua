...
function Inventory:InitUi()
	...
	uiFrame:SetScript("OnUpdate", function()
		local spellTargeting = (_G.SpellIsTargeting and _G.SpellIsTargeting()) and true or false
		if _G.this.bagshuiData.lastSpellTargeting ~= spellTargeting then
			_G.this.bagshuiData.lastSpellTargeting = spellTargeting
			self:UpdateSecureItemTargetingButtons(true)
		end
		self:UpdateSecureRightClickButtons()
	...
	buttons.toolbar.hearthstone:SetAttribute("type1", nil)
	buttons.toolbar.hearthstone:SetAttribute("macrotext1", nil)
	...
	if type(buttons.toolbar.disenchant.SetAttribute) == "function" then
		buttons.toolbar.disenchant:SetAttribute("type", "spell")
		buttons.toolbar.disenchant:SetAttribute("spell", L.Spell_Disenchant)
	end
	if type(buttons.toolbar.disenchant.HookScript) == "function" then
		buttons.toolbar.disenchant:HookScript("OnClick", function()
			Bagshui:QueueClassCallback(self, self.UpdateSecureItemTargetingButtons, nil, nil, true)
		end)
	end
	...
	if type(buttons.toolbar.pickLock.SetAttribute) == "function" then
		buttons.toolbar.pickLock:SetAttribute("type", "spell")
		buttons.toolbar.pickLock:SetAttribute("spell", L.Spell_PickLock)
	end
	if type(buttons.toolbar.pickLock.HookScript) == "function" then
		buttons.toolbar.pickLock:HookScript("OnClick", function()
			Bagshui:QueueClassCallback(self, self.UpdateSecureItemTargetingButtons, nil, nil, true)
		end)
	end
...
function Inventory:UiFrame_OnHide()
	...
	if not self.dockTo and self.uiFrame:GetTop() then
...
function Inventory:Open()
	local currentEvent = Bagshui.currentEvent
	if not self.uiFrame:IsVisible() then
		...
		self.lastOpenEventTrigger =
			currentEvent
			and (
				(string.find(currentEvent, "_OPENED$")) and (string.gsub(currentEvent, "OPENED$", ""))
				or (string.find(currentEvent, "_SHOW$")) and (string.gsub(currentEvent, "SHOW$", ""))
			)
			or nil
	end
	self.uiFrame:Show()
...
function Inventory:Close()
	...
	if self.uiFrame:IsVisible() then
		...
		if
			type(currentEvent) == "string"
			and (string.find(currentEvent, "_CLOSED$"))
			and (
				self.lastOpenEventTrigger == nil
				or (not string.find(currentEvent, "^" .. self.lastOpenEventTrigger))
			)
		then
			return
		end
