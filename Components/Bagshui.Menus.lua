...
---@param arg8 any? Additional original client parameter that must be forwarded to Blizzard's `ToggleDropDownMenu()`.
---@param arg9 any? Additional original client parameter that must be forwarded to Blizzard's `ToggleDropDownMenu()`.
---@param arg10 any? Additional original client parameter that must be forwarded to Blizzard's `ToggleDropDownMenu()`.
...
function Bagshui:ToggleDropDownMenu(wowApiFunctionName, level, value, dropDownFrame, anchorName, xOffset, yOffset, arg8, arg9, arg10, bagshuiAdditionalPassReason)
	...
	if phase == 0 then
		self.hooks:OriginalHook(wowApiFunctionName, level, value, dropDownFrame, anchorName, xOffset, yOffset, arg8, arg9, arg10)
...
function Bagshui:UIDropDownMenu_AddButton(wowApiFunctionName, menuItem, level, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10)
	local dropdown = level
	local resolvedLevel = arg3

	-- Some clients expect UIDropDownMenu_AddButton(info, dropdown, level, ...)
	-- while Bagshui and many classic addons call it as (info, level). A number
	-- is never a valid dropdown frame, so always treat a numeric 2nd parameter
	-- as the level and move it out of the dropdown position.
	if type(level) == "number" then
		dropdown = nil
		resolvedLevel = level
	end

	self.hooks:OriginalHook(wowApiFunctionName, menuItem, dropdown, resolvedLevel, arg4, arg5, arg6, arg7, arg8, arg9, arg10)
	...
	if type(resolvedLevel) ~= "number" then
		if type(arg3) == "number" then
			resolvedLevel = arg3
		else
			resolvedLevel = tonumber(level) or _G.UIDROPDOWNMENU_MENU_LEVEL or 1
		end
	end
