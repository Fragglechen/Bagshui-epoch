...
function ObjectEditor:PromptForSave()
	...
	if not _G.StaticPopupDialogs[dialogName] then
		_G.StaticPopupDialogs[dialogName] = {
			...
			-- StaticPopup passes the dialog/button context here, so the editor must be read from dialog.data.
			OnAccept = function(dialog)
				local editor = dialog and dialog.data or dialog
				if not editor or not editor.uiFrame or not editor.uiFrame.bagshuiData then
					return
				end
