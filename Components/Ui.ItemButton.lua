...
---@param inherits string? Optional XML template(s) to inherit.
---@return table itemSlotButton
function Ui:CreateItemSlotButton(name, parent, inherits)
	...
	local button = _G.CreateFrame(
		...
		parent,
		inherits or "ItemButtonTemplate"
	)
...
function Ui:CreateItemButtonCooldown(button, disableText)
	...
	-- Newer clients route CooldownFrame_SetTimer() through cooldown:SetCooldown().
	-- The older Bagshui model-based cooldown doesn't provide that method, so add
	-- a compatibility shim that drives the legacy model properties.
	if not cooldown.SetCooldown then
		cooldown.SetCooldown = function(self, start, duration)
			if (start or 0) > 0 and (duration or 0) > 0 then
				self.start = start
