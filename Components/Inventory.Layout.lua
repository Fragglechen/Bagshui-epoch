	...
	if groupItemCount > 0 then
		...
		for position = firstItem, lastItem, step do
			...
			if not hideItem then
				...
				self:UpdateSecureItemSlotButtonAttributes(button)
...
function Inventory:UpdateToolbar()
	...
	then
		...
		if type(toolbarButtons.hearthstone.SetAttribute) == "function" then
			toolbarButtons.hearthstone:SetAttribute("type1", "macro")
			toolbarButtons.hearthstone:SetAttribute(
				"macrotext1",
				"/use " .. tostring(self.hearthstoneItemRef.bagNum) .. " " .. tostring(self.hearthstoneItemRef.slotNum)
			)
		end
		...
		if type(toolbarButtons.hearthstone.SetAttribute) == "function" then
			toolbarButtons.hearthstone:SetAttribute("type1", nil)
			toolbarButtons.hearthstone:SetAttribute("macrotext1", nil)
		end
...
function Inventory:ItemSlotAndGroupMouseOverCheck()
	...
	if self.editMode and not mouseOverFound then
		for _, group in pairs(self.ui.frames.groups) do
