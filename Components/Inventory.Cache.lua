...
function Inventory:UpdateCache()
	...
	for _bagIndex, bagNum in ipairs(self.containerIds) do
		...
		if self.primaryContainer.id == bagNum then
			...
			local inventoryId = _G.ContainerIDToInventoryID(bagNum)
			bagSlotLink = inventoryId and _G.GetInventoryItemLink("player", inventoryId)
		...
		local inventoryId = _G.ContainerIDToInventoryID(bagNum)
		self.lastUpdateLockedContainers[bagNum] = inventoryId and _G.IsInventoryItemLocked(inventoryId) or nil
			...
			for slotNum = 1, bagNumSlots do
				...
				then
					...
					if item.itemString ~= nil then
						...
						if self.containers[bagNum].isProfessionBag and item.bagType then
...
function Inventory:AddItemBagInfo(item, bagInfo, isEmptySlotStack)
	...
	item.bagType = isEmptySlotStack and bagInfo.genericType or bagInfo.type or bagInfo.genericType
end
