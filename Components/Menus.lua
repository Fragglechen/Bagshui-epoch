...
function Menus:Init()
	...
	---@param valueOrParams any value or arg1 from the menu `func()` call, depending on client signature.
	---@param paramsOrItemId any arg1 or arg2 from the menu `func()` call, depending on client signature.
	---@param itemIdOrChecked any arg2 or checked state from the menu `func()` call, depending on client signature.
	self._autoSplitMenuItem_Click = function(valueOrParams, paramsOrItemId, itemIdOrChecked)
		local params
		local itemId

		if type(valueOrParams) == "table" and valueOrParams.menuItem then
			params = valueOrParams
			itemId = paramsOrItemId
		elseif type(paramsOrItemId) == "table" and paramsOrItemId.menuItem then
			params = paramsOrItemId
			itemId = itemIdOrChecked
		else
			params = valueOrParams
			itemId = paramsOrItemId
		end
...
function Menus:ShowMenu(menuType, anchorFrame, xOffset, yOffset, anchorPoint, anchorToPoint)
	...
	_G.UIDropDownMenu_Initialize(
		Bagshui.menuFrame,
		function(arg1, arg2)
			local level = type(arg1) == "number" and arg1 or arg2
