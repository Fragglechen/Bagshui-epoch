...
function Inventory:MoveItem(source, target, onComplete)
	...
	elseif type(target) == "number" then
		...
		local inventoryId = _G.ContainerIDToInventoryID(target)
		if inventoryId then
...
function Inventory:UIErrorsFrame_OnEvent(wowApiFunctionName, event, message)
	...
	then
		...
		for _, containerId in ipairs(self.containerIds) do
			local inventoryId = _G.ContainerIDToInventoryID(containerId)
			if inventoryId and _G.IsInventoryItemLocked(inventoryId) then
				bagSlot = containerId
			end
		end
