	...
	then
		...
		if not _G.StaticPopupDialogs[dialogName] then
			_G.StaticPopupDialogs[dialogName] = {
				...
				---@param dialogOrData table StaticPopup dialog frame or callback data table.
				OnAccept = function(dialogOrData)
					local data = dialogOrData and dialogOrData.data or dialogOrData
					if data and type(data.disableFunc) == "function" then
				...
				--- Ignore was clicked.
				---@param dialogOrData table StaticPopup dialog frame or callback data table.
				OnCancel = function(dialogOrData, reason)
					local data = dialogOrData and dialogOrData.data or dialogOrData
					if reason == "override" then
						return
					end
					if data and type(data.ignoreFunc) == "function" then
