...
function Bags:SmartOpenClose(action)
	...
	if
		...
		and (
			(string.find(currentEvent, "^AUCTION_HOUSE_") and self.settings.toggleBagsWithAuctionHouse == false)
			or
			(string.find(currentEvent, "^BANKFRAME_") and self.settings.toggleBagsWithBankFrame == false)
			or
			(string.find(currentEvent, "^MAIL_") and self.settings.toggleBagsWithMailFrame == false)
			or
			(string.find(currentEvent, "^TRADE_") and self.settings.toggleBagsWithTradeFrame == false)
		)
	then
		return
	end
