...
function InventoryUi:CreateInventoryItemSlotButton(buttonNum)
	...
	ui:CreateIfNotExists(
		...
		function(elementNum)
			...
			if type(slotButton.SetAttribute) == "function" then
				slotButton:SetAttribute("type1", nil)
			end
			slotButton.bagshuiData.defaultOnClick = inventory._itemSlotButton_ScriptWrapper_OnClick
			slotButton.bagshuiData.defaultOnDragStart = inventory._itemSlotButton_ScriptWrapper_OnDragStart
			slotButton.bagshuiData.defaultOnReceiveDrag = inventory._itemSlotButton_ScriptWrapper_OnReceiveDrag

			self:AddItemSlotButtonGetIdProxy(slotButton)
			self:AddSecureTargetOverlay(slotButton, elementNum)
			self:AddSecureRightClickOverlay(slotButton, elementNum)
		end
	)
end


--- Create an invisible real container-slot button that can temporarily sit on top
--- of a Bagshui slot while a spell is targeting. This gives the client a true
--- `ContainerFrameItemButtonTemplate` click target with its own parent bag ID.
---@param slotButton table
---@param elementNum number
function InventoryUi:AddSecureTargetOverlay(slotButton, elementNum)
	local holder = _G.CreateFrame(
		"Frame",
		self:CreateElementName("ItemSecureTargetHolder" .. elementNum),
		slotButton
	)
	holder:SetAllPoints(slotButton)
	holder:SetFrameLevel(slotButton:GetFrameLevel() + 20)
	holder:Hide()
	holder:EnableMouse(false)
	slotButton.bagshuiData.secureTargetHolder = holder

	local targetButton = _G.CreateFrame(
		"Button",
		self:CreateElementName("ItemSecureTargetButton" .. elementNum),
		holder,
		"ContainerFrameItemButtonTemplate"
	)
	targetButton:ClearAllPoints()
	targetButton:SetAllPoints(holder)
	targetButton:SetFrameLevel(holder:GetFrameLevel() + 1)
	targetButton:RegisterForClicks("LeftButtonUp")
	targetButton:EnableMouse(false)
	targetButton:SetAlpha(0)
	targetButton:Hide()
	slotButton.bagshuiData.secureTargetButton = targetButton
end


--- Create an invisible secure right-click button that can temporarily sit on top
--- of a Bagshui slot when the plain "use item" right-click path should be active.
---@param slotButton table
---@param elementNum number
function InventoryUi:AddSecureRightClickOverlay(slotButton, elementNum)
	local holder = _G.CreateFrame(
		"Frame",
		self:CreateElementName("ItemSecureRightClickHolder" .. elementNum),
		slotButton
	)
	holder:SetAllPoints(slotButton)
	holder:SetFrameLevel(slotButton:GetFrameLevel() + 19)
	holder:Hide()
	holder:EnableMouse(false)
	slotButton.bagshuiData.secureRightClickHolder = holder

	local button = _G.CreateFrame(
		"Button",
		self:CreateElementName("ItemSecureRightClickButton" .. elementNum),
		holder,
	...
	button:SetScript("OnEnter", function()
		self.inventory:ItemButton_OnEnter(slotButton)
	end)
	button:SetScript("OnLeave", function()
		self.inventory:ItemButton_OnLeave(slotButton)
	end)
	slotButton.bagshuiData.secureRightClickButton = button
end


--- Keep the secure bag/slot attributes of an item button in sync with its assigned item.
---@param itemButton table
function Inventory:UpdateSecureItemSlotButtonAttributes(itemButton)
	if _G.UnitAffectingCombat and _G.UnitAffectingCombat("player") then
		return
	end

	if not itemButton or type(itemButton.SetAttribute) ~= "function" then
		return
	end

	local bagNum = itemButton.bagshuiData and itemButton.bagshuiData.bagNum
	local slotNum = itemButton.bagshuiData and itemButton.bagshuiData.slotNum

	if bagNum ~= nil and slotNum ~= nil then
		if itemButton.bagshuiData.secureTargetHolder then
			itemButton.bagshuiData.secureTargetHolder:SetID(bagNum)
		end
		if itemButton.bagshuiData.secureTargetButton then
			itemButton.bagshuiData.secureTargetButton:SetID(slotNum)
		end
		if itemButton.bagshuiData.secureRightClickButton then
			itemButton.bagshuiData.secureRightClickButton:SetAttribute("type", "item")
			itemButton.bagshuiData.secureRightClickButton:SetAttribute("item", string.format("%d %d", bagNum, slotNum))
		end
	else
		if itemButton.bagshuiData.secureTargetHolder then
			itemButton.bagshuiData.secureTargetHolder:SetID(0)
		end
		if itemButton.bagshuiData.secureTargetButton then
			itemButton.bagshuiData.secureTargetButton:SetID(0)
		end
		if itemButton.bagshuiData.secureRightClickButton then
			itemButton.bagshuiData.secureRightClickButton:SetAttribute("type", nil)
			itemButton.bagshuiData.secureRightClickButton:SetAttribute("item", nil)
		end
	end
end


--- Determine whether the secure right-click "use item" overlay should be active.
---@return boolean
function Inventory:IsSecureRightClickUseAllowed()
	return (
		not self.editMode
		and self.online
		and not (_G.SpellIsTargeting and _G.SpellIsTargeting())
		...
		and not self.ui:IsFrameVisible("SendMailFrame")
		and not self.ui:IsFrameVisible("AuctionFrame")
		and not self.ui:IsFrameVisible("TradeFrame")
		and not self.ui:IsFrameVisible("aux_frame")
		and not self.ui:IsFrameVisible("CT_MailFrame")
		and not self.ui:IsFrameVisible("PostalFrame")
	)
end


--- Enable or disable the secure right-click "use item" overlay on inventory buttons.
---@param force boolean?
function Inventory:UpdateSecureRightClickButtons(force)
	if _G.UnitAffectingCombat and _G.UnitAffectingCombat("player") then
		return
	end

	if not self.ui or not self.ui.buttons or not self.ui.buttons.itemSlots then
		return
	end

	local rightClickUseAllowed = self:IsSecureRightClickUseAllowed()
	if not force and self._secureRightClickUseState == rightClickUseAllowed then
		return
	end
	self._secureRightClickUseState = rightClickUseAllowed

	for _, itemButton in ipairs(self.ui.buttons.itemSlots) do
		if itemButton and itemButton.bagshuiData then
			local rightClickHolder = itemButton.bagshuiData.secureRightClickHolder
			local rightClickButton = itemButton.bagshuiData.secureRightClickButton
			local item = itemButton.bagshuiData.item
			local enable = rightClickUseAllowed and item and item.itemString ~= nil

			if rightClickHolder and rightClickButton then
				if enable then
					rightClickHolder:Show()
					rightClickHolder:EnableMouse(false)
					rightClickButton:Show()
					rightClickButton:EnableMouse(true)
				else
					rightClickButton:EnableMouse(false)
					rightClickButton:Hide()
					rightClickHolder:EnableMouse(false)
					rightClickHolder:Hide()
				end
			end
		end
	end
...
--- Enable or disable the secure item-targeting click path on inventory buttons.
---@param force boolean?
function Inventory:UpdateSecureItemTargetingButtons(force)
	if _G.UnitAffectingCombat and _G.UnitAffectingCombat("player") then
		return
	end
	...
	local targetingSpell = (_G.SpellIsTargeting and _G.SpellIsTargeting()) and true or false
	if not force and self._secureItemTargetingState == targetingSpell then
		return
	end
	self._secureItemTargetingState = targetingSpell

	for _, itemButton in ipairs(self.ui.buttons.itemSlots) do
		if itemButton and type(itemButton.SetAttribute) == "function" then
			self:UpdateSecureItemSlotButtonAttributes(itemButton)
			itemButton:SetAttribute("type1", nil)
			if itemButton.bagshuiData.secureTargetHolder and itemButton.bagshuiData.secureTargetButton then
				if targetingSpell then
					itemButton.bagshuiData.secureTargetHolder:Show()
					itemButton.bagshuiData.secureTargetHolder:EnableMouse(false)
					itemButton.bagshuiData.secureTargetButton:Show()
					itemButton.bagshuiData.secureTargetButton:EnableMouse(true)
				else
					itemButton.bagshuiData.secureTargetButton:EnableMouse(false)
					itemButton.bagshuiData.secureTargetButton:Hide()
					itemButton.bagshuiData.secureTargetHolder:EnableMouse(false)
					itemButton.bagshuiData.secureTargetHolder:Hide()
				end
			end
		end
	end
...
function Inventory:ItemButton_OnEnter(itemButton)
	...
	-- Protected spell targeting such as Disenchant must not run through Bagshui's
	-- tooltip/container mouseover path because that path temporarily rewrites
	-- global `this` for compatibility, which taints the click flow.
		...
		if item.emptySlot == 1 then
			...
			if truncateTooltip then
				...
				if firstBlankLine then
					...
					if _G.GameTooltipMoneyFrame and _G.GameTooltipMoneyFrame.Hide then
...
function Inventory:ItemButton_OnClick(mouseButton, isDrag)
	...
	if self.editMode then
		...
		if
			...
			elseif
				-- While a spell is targeting, Bagshui must not route any click path through
				-- its normal Lua item handling. The secure slot button attributes are
				-- responsible for the actual left-click targeting.
				_G.SpellIsTargeting
				and _G.SpellIsTargeting()
			then
				self:UpdateSecureItemTargetingButtons(true)
				return
			...
			then
				...
				then
					...
					local clickFrame = itemButton.bagshuiData.getIdProxy or itemButton
					_G.this = clickFrame  -- Keep Vanilla-style consumers working too.
					_G.IsAltKeyDown = BsUtil.ReturnTrue
					_G.Aux_ContainerFrameItemButton_OnClick(clickFrame, "LeftButton")
			...
			if not clickHandled then
				...
				if mouseButton == "LeftButton" then
					...
					local clickFrame = itemButton.bagshuiData.getIdProxy or itemButton
					_G.ContainerFrameItemButton_OnClick(clickFrame, mouseButton)
				end
				-- Restore global this.
				_G.this = oldGlobalThis
			end
